...
glm::mat4 cameraToWorld(const Camera& cam)
{
  ...
  return Mc;
  */
  return glm::inverse(glm::lookAt(cam.eye(), cam.eye() + -cam.back_vector, cam.up_vector));
}
...
glm::mat4 cameraToNDC(Camera& cam, float windowX, float windowY, float nearPlane, float farPlane)
{
  /*
  glm::mat4 M(1.0f);
  //W,H,D
  ...
  */
  ...
  return glm::perspective(cam.zoomScale, windowX/windowY, nearPlane, farPlane);
